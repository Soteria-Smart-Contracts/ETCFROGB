...
async function UpdateTotalEarned(){
    ...
    //convert the total from wei to eth and round to 2 decimal places using tofixed in one line
...
async function DisplayNFT(ID){
    ...
    NewNFT.innerHTML = img + '<br>' + '<a class="nftID">FrogB ID:</a>' + '<br>' + '<a class="nftID">' + ID + '</a>' + '<br>' + '<a class="nftID scout" href="' + ScoutLink + '" target="_blank">View On Blockscout</a>' +  '<br><br>' + '<button class="ButtonReg">Claim Rewards</button>' //add a button thats says claim;
